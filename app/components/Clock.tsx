...
const Clock: React.FC<ClockProps> = ({ size = 200, style }) => {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={style}>
      ...
      {/* Hour hand */}
      <line x1={size/2} y1={size/2} x2={size/2} y2={size/2 - size*0.25}
        stroke="#333" strokeWidth="6" strokeLinecap="round"
        transform={`rotate(${hourDeg} ${size/2} ${size/2})`} />
      {/* Minute hand */}
      <line x1={size/2} y1={size/2} x2={size/2} y2={size/2 - size*0.35}
        stroke="#333" strokeWidth="4" strokeLinecap="round"
        transform={`rotate(${minDeg} ${size/2} ${size/2})`} />
      {/* Second hand */}
      <line x1={size/2} y1={size/2} x2={size/2} y2={size/2 - size*0.42}
        stroke="#e00" strokeWidth="2" strokeLinecap="round"
        transform={`rotate(${secDeg} ${size/2} ${size/2})`} />
